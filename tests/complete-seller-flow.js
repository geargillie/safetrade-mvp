...
async function testCompleteSellerFlow() {
  ...
  try {
    ...
    // STEP 3: Create conversation and simulate buyer message
    ...
    // STEP 6: Test listing status management
    ...
    if (statusError) {
      console.log('❌ Step 6 FAILED: Could not update listing status -', statusError.message);
      flowResults.push('Step 6: FAILED - Listing status error');
    } else {
      console.log('✅ Step 6 PASSED: Listing status updated to in_talks');
